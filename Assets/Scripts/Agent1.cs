...
public class Agent1 : MonoBehaviour
{
    ...
    void Start()
    {
        dbFilePath = Path.Combine(Application.dataPath, "Agent1.db");
        ...
        while (true)
        {
            if (!isMoving)
            {
                if (hasGold && targetPosition == initialPosition)
                {
                    ...
                    gridGenerator.ContadorIteracoes(0);
                    Destroy(gameObject);
                    yield break;
                }
            //VERIFICAR ESTE BLOCO DE CODIGOS
                RandomDirection();
   ...
   void Move(Vector2Int direction, string directionName)
{
    ...
    // Verifique se a nova posição está dentro dos limites do grid
    ...
    void RandomDirection()
{
    ...
    Vector2Int directionVector = Vector2Int.zero; // Inicializando com um valor padrão
    string directionName = ""; // Inicializando com um valor padrão
   ...
   void BreezerCondition()
{
    ...
    // 50% de chance de retornar à casa anterior
    if (chance < 50)
    {
        Vector3 direction = previousPosition - targetPosition;
        string directionName = "";

        if (direction == Vector3.up * spacing)
        {
            directionName = "N";
        }
        else if (direction == Vector3.down * spacing)
        {
            directionName = "S";
        }
        else if (direction == Vector3.left * spacing)
        {
            directionName = "O";
        }
        else if (direction == Vector3.right * spacing)
        {
            directionName = "L";
        }

        targetPosition = previousPosition; // Move para a posição anterior
        LogRegister("...", "Move", directionName); // Registra a ação de retornar
        isMoving = true;
    }
